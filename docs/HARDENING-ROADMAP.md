...
**Status:** PRs #34–#36 merged; E2E suite split in review  
...
- Split the large `core-flow.spec.ts` into focused specs (onboarding, session lifecycle, history/data, behaviour guidance, install/public) to reduce merge-conflict risk. **In review.**
...
- Manual history add, edit and delete.
- Explicit early return from a timed departure, save, and resulting next-plan behaviour.
- Progress screen after a mixture of relaxed / concern / distressed sessions.
- Backup **export + restore round-trip**, not restore alone.
- Track/scenario settings: duration entry, minutes/seconds handling, warm-up count, shuffle and rest settings.
...
- Account-aware copy: local-only users and connected users must not be told contradictory things about backup/sync.
...
4. Split E2E specs to reduce conflict risk. **In review.**
